...
async def ensure_end_device(  # noqa C901
    ...
    has_registration_link: bool | None = instruction.parameters.get(
    ...
    if is_aggregator:
        ...
        if aggregator_id is None:
            ...
            cert = (
                await session.execute(
                    select(Certificate).where(
                        Certificate.lfdi == client_config.lfdi.lower()
                    )
                )
            ).scalar_one_or_none()
            if cert is None:
            ...
            aggregator_id = await find_aggregator_id(
                client_config.lfdi, context, session
            )
            if aggregator_id is None:
                return ActionResult.failed(
            ...
            session.add(
                AggregatorCertificateAssignment(
                    certificate_id=cert.certificate_id, aggregator_id=aggregator_id
                )
            )
            await session.flush()
            logger.info(
    ...
    stmt = select(Site).where(
        (Site.aggregator_id == aggregator_id) & (Site.lfdi == client_config.lfdi)
    )
    ...
    if registered:
        ...
        if existing is None:
            ...
            site = Site(
                ...
                // 10,  # config holds 6-digit SEP2 PIN; envoy stores 5-digit raw PIN
        ...
        if site_created:
            await NotificationManager.notify_changed_deleted_entities(
                SubscriptionResource.SITE, site_changed_time
            )
        return ActionResult.done()
    else:
        if existing is None:
...
async def delete_site(site_id: int, session: AsyncSession) -> None:
    ...
    if srt_ids:
        await session.execute(
            delete(SiteReading).where(SiteReading.site_reading_type_id.in_(srt_ids))
        )
    await session.execute(
        delete(SiteReadingType).where(SiteReadingType.site_id == site_id)
    )

    # subscriptions (scoped_site_id is nullable FK, no cascade); SubscriptionCondition cascades from subscription
    await session.execute(
        delete(Subscription).where(Subscription.scoped_site_id == site_id)
    )

    # tariff rates and DOEs (no cascade)
    await session.execute(
        delete(TariffGeneratedRate).where(TariffGeneratedRate.site_id == site_id)
    )
    await session.execute(
...
async def _ensure_site_der(site_id: int, session: AsyncSession) -> None:
    ...
    """
    existing = (
        await session.execute(select(SiteDER).where(SiteDER.site_id == site_id))
    ).scalar_one_or_none()
    if existing is None:
